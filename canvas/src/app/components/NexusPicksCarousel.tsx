      ...
      <Box className={styles['carousel-inner']} sx={{ display: 'flex', alignItems: 'center', width: '100%', maxWidth: 1200, position: 'relative' }}>
        ...
        <Box className={styles['carousel-cards']} sx={{ display: 'flex', gap: 4, flex: 1, overflow: 'hidden' }}>
          {displayedCards.map((card) => (
            <Paper key={card.renderKey} elevation={2} className={styles['carousel-card']} sx={{ width: cardSize, height: cardSize, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: '#f5f5f5', transition: 'width 0.2s, height 0.2s' }}>
